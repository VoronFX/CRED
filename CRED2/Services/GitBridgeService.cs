    ...
    public sealed class GitBridgeService : IDisposable
    {
            ...
            Dispatcher.Post(async () =>
                    {
                        ...
                        foreach (var branch in branches.Where(x => !x.Broken))
                            await RegisterBranch(branch.Id);
                    });
        }
        ...
        public async Task RegisterBranch(long id)
        {
            ...
            try
            {
                ...
                {
                    Logger.LogWarning("Remote configuration mismatch branch {BranchName}, branch id {BranchId}. Removing...", branch.Name, branch.Id);
                    Repository.Network.Remotes.Remove(branch.Id.ToString());
                    remote = null;
                }
                ...
                if (remote == null)
                {
                    Logger.LogInformation("Adding remote for branch {BranchName}, branch id {BranchId}", branch.Name, branch.Id);
            ...
            {
                Logger.LogError("Branch registration failed. Branch name {BranchName}, branch id {BranchId}.", branch.Name, branch.Id);
                branch.Broken = true;
        ...
        {
            var branch = Database.SingleById<Branch>(id);
            Repository.Network.Fetch(branch.Id.ToString(),
            ...
            if (Repository.Branches[branch.Id.ToString()].Tip.Sha != commit.Hash)
            {
                Logger.LogInformation(
                    "Updating branch {BranchName}, branch id {BranchId} from {LastSyncedCommitHash} to {NewCommitHash}", branch.Name, branch.Id,
                    commit.Hash, Repository.Branches[branch.Id.ToString()].Tip.Sha);
                ...
                try
                {
                    if (Repository.Branches[branch.Id.ToString()].Commits.All(x => x.Sha != commit.Hash))
                    {
                        ...
                        {
                            Logger.LogWarning("Non fast-forward update of branch {BranchName}, branch id {BranchId}", branch.Name, branch.Id);
                        }
                        else
                        {
                    ...
                    using (var transaction = new Transaction(Database))
                    {
                        ...
                        var newCommit = await EnsureChangesCalculated(Repository.Branches[branch.Id.ToString()].Tip, transaction);
                        transaction.Commit();
                        branch.CommitId = newCommit.Id;
                        BranchNextUpdate[branch.Id] = DateTime.UtcNow + TimeSpan.FromMinutes(1);
                        Logger.LogInformation("Update of branch {BranchName}, branch id {BranchId} complete", branch.Name, branch.Id);
                    }
                }
                catch (Exception e)
                {
                    Logger.LogError(e, "Update of branch {BranchName}, branch id {BranchId} failed", branch.Name, branch.Id);
                    branch.Broken = true;
            ...
            {
                Logger.LogInformation("Branch {BranchName}, branch id {BranchId} is up to date", branch.Name, branch.Id);
            }
        }
