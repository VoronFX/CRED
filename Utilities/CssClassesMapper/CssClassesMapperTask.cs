...
using CssClassesMapper;

namespace CssClassesMapper
{
	public partial class CssClassesMapper
	{
		private static Action<CssClassesMapperTask> Execute { get; set; }
		...
		private static void Main(string[] args)
		{
			using (xmlWriter = XmlWriter.Create(Console.Out))
			{
				...
				try
				{
					...
					{
						Execute((CssClassesMapperTask)CssClassesMapperTask.Serializer().Deserialize(stream));
					}
				}
				catch (Exception e)
				{
	...
	[Serializable]
	public class CssClassesMapperTask : Task
	{
		[Required]
		[XmlElement]
        ...
        public static XmlSerializer Serializer()
		{
			...
			foreach (var prop in typeof(CssClassesMapperTask)
				.GetProperties()
			...
			return new XmlSerializer(typeof(CssClassesMapperTask), overrides);
		}
			...
			try
			{
				var process = new Process
				{
					StartInfo =
					{
						...
						FileName = Path.Combine(TaskAssemblyPath, "CssClassesMapper.exe"),
					}
				};
				process.Start();
				var success = true;
